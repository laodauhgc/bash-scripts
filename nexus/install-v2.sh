...
# Version: v1.7.0 | Update 16/08/2025
...
CLI_TAG_FILE="$STATE_DIR/cli_tag.txt"    # tag CLI đã build gần nhất
REPLICA_FILE="$STATE_DIR/replicas.txt"   # số container nhân bản
WALLET_FILE="$STATE_DIR/wallet.txt"      # lưu wallet

# Mặc định: luôn set cron
SETUP_CRON=1
...
# Nhận đầu vào ban đầu (có thể là wallet hoặc node-id)
PRIMARY_INPUT="${1-}"
...
# ===================== Bắt cờ ngôn ngữ sớm (không tiêu thụ tham số) =====================
for arg in "$@"; do
  case "$arg" in
    --en) LANGUAGE="en" ;; --ru) LANGUAGE="ru" ;; --cn) LANGUAGE="cn" ;;
  esac
done

# ===================== Thông điệp ngắn gọn =====================
case $LANGUAGE in
  vi) BANNER="===== Cài Đặt Node Nexus v1.7.0 ====="
      USE_INFO_CRON="Thiết lập cron: watchdog mỗi 5' và kiểm tra phiên bản mới mỗi 1h (chỉ cập nhật khi có tag mới)."
      CRON_DONE="Đã thiết lập cron."
      ERR_NO_INPUT="Lỗi: Cần cung cấp ít nhất wallet hoặc node id.\nCách dùng: $0 [<wallet|node-id>] [-n <số> | --nodes <số>] [--wallet <addr>] [--node-id <id>]"
      PROMPT_NODE_ID="API Nexus có thể lỗi khi đăng ký. Vui lòng nhập NODE_ID của bạn (Enter để bỏ qua): "
      ;;
  *)  BANNER="===== Nexus Node Setup v1.7.0 ====="
      USE_INFO_CRON="Cron: watchdog every 5m and check new release hourly (update only on new tag)."
      CRON_DONE="Cron configured."
      ERR_NO_INPUT="Error: Provide at least a wallet or node id.\nUsage: $0 [<wallet|node-id>] [-n <num> | --nodes <num>] [--wallet <addr>] [--node-id <id>]"
      PROMPT_NODE_ID="Nexus API may fail on registration. Please input your NODE_ID (press Enter to skip): "
      ;;
esac
inf "$BANNER"
...
# ===================== Parse flags =====================
# Ta sẽ:
#  - lấy PRIMARY_INPUT (nếu có)
#  - parse --wallet/--node-id
#  - parse -n/--nodes (nếu truyền thì phải số > 1)
#  - các mode --watchdog/--update
shift || true  # bỏ PRIMARY_INPUT khỏi $@
WALLET_ADDRESS=""
NODE_ID_CLI=""
NO_SWAP=0

is_number(){ [[ "$1" =~ ^[0-9]+$ ]]; }
is_wallet_like(){ [[ "$1" =~ ^0x[0-9a-fA-F]{38,}$ ]]; }  # rộng rãi
# node id có thể đa dạng -> nếu không giống wallet thì coi như node-id
deduce_primary(){
  local s="$1"
  [ -z "$s" ] && return 0
  if is_wallet_like "$s"; then WALLET_ADDRESS="$s"; else NODE_ID_CLI="$s"; fi
}
deduce_primary "$PRIMARY_INPUT"

while [ $# -gt 0 ]; do
  case "$1" in
    --wallet) WALLET_ADDRESS="${2-}"; shift 2 ;;
    --wallet=*) WALLET_ADDRESS="${1#--wallet=}"; shift ;;
    --node-id) NODE_ID_CLI="${2-}"; shift 2 ;;
    --node-id=*) NODE_ID_CLI="${1#--node-id=}"; shift ;;
    -n) 
      [ -z "${2-}" ] && { err "Thiếu giá trị cho -n"; exit 1; }
      is_number "$2" && [ "$2" -gt 1 ] || { err "-n phải là số > 1"; exit 1; }
      REPLICAS="$2"; shift 2 ;;
    -n=*)
      val="${1#-n=}"; is_number "$val" && [ "$val" -gt 1 ] || { err "-n phải là số > 1"; exit 1; }
      REPLICAS="$val"; shift ;;
    --nodes)
      [ -z "${2-}" ] && { err "Thiếu giá trị cho --nodes"; exit 1; }
      is_number "$2" && [ "$2" -gt 1 ] || { err "--nodes phải là số > 1"; exit 1; }
      REPLICAS="$2"; shift 2 ;;
    --nodes=*)
      val="${1#--nodes=}"; is_number "$val" && [ "$val" -gt 1 ] || { err "--nodes phải là số > 1"; exit 1; }
      REPLICAS="$val"; shift ;;
    --no-swap) NO_SWAP=1; shift ;;
    --watchdog) MODE="watchdog"; shift ;;
    --smart-update|--update) MODE="update"; shift ;;
    --setup-cron) shift ;;   # giữ tương thích; nhưng luôn bật mặc định
    --en|--ru|--cn) shift ;; # đã xử lý sớm
    *)
      warn "Bỏ qua flag không hợp lệ: $1"; shift ;;
  esac
done
...
# Lưu state đầu vào để cron dùng về sau
[ -n "$WALLET_ADDRESS" ] && echo -n "$WALLET_ADDRESS" > "$WALLET_FILE"
if [ -n "$NODE_ID_CLI" ]; then
  echo -n "$NODE_ID_CLI" > "$NODE_ID_FILE"
fi
# Lưu replicas (mặc định 1; nếu có truyền hợp lệ >1 thì giữ >1)
...
[ "${NO_SWAP:-0}" = 1 ] && warn "Bỏ qua tạo swap theo yêu cầu (--no-swap)." || create_swap
...
EOF
  ...
  # ENTRYPOINT: chạy headless, không TUI để tránh /dev/tty
...
detect_cpus() {
  ...
  if [ -r /sys/fs/cgroup/cpu.max ]; then
    ...
    if [ "${quota:-max}" != "max" ] && [ -n "$quota" ] && [ -n "$period" ] && [ "$period" -gt 0 ]; then
      ...
      [ "$ceil" -gt 0 ] && [ "$ceil" -lt "$cpus" ] && cpus=$ceil
    fi
...
# Chạy headless để tránh lỗi /dev/tty
...
container_name_for_index(){
  ...
  local w; w="$(pad_width "$total")"
  printf "%s-%0${w}d" "$CONTAINER_NAME" "$idx"
}
log_file_for_index(){
  local idx="$1" total="$2"
  if [ "$total" -le 1 ]; then echo "$LOG_FILE"; return; fi
  local w; w="$(pad_width "$total")"
  printf "%s/%s-%0${w}d.log" "$LOG_DIR" "$CONTAINER_NAME" "$idx"
}

# Lấy replicas từ file nếu có (cron dùng)
get_replicas(){
  local n=""
  [ -f "$REPLICA_FILE" ] && n="$(tr -d ' \t\r\n' < "$REPLICA_FILE" 2>/dev/null || true)"
  case "$n" in ''|*[!0-9]*) n="$REPLICAS" ;; esac
  [ -z "$n" ] && n=1
  echo "$n"
}

# ===================== Run 1 container instance =====================
run_container_instance() {
  ...
  # Lấy WALLET/NODE_ID hiệu lực: ưu tiên CLI -> state -> credentials
  ...
  # Nếu cả WALLET và NODE_ID đều trống => không thể chạy
  ...
  # Nếu chưa có NODE_ID thì chờ entrypoint tạo; nếu vẫn không có, hỏi tay (chỉ ở MODE normal)
  if [ -z "$NODE_ID" ]; then
    ...
    if [ -z "$NODE_ID" ]; then
      if [ "$MODE" = "normal" ]; then
        ...
        if [ -n "$NODE_MANUAL" ]; then
          echo -n "$NODE_MANUAL" > "$NODE_ID_FILE"
          ok "Đã nhận node id thủ công."
          # Rerun instance với node id vừa nhập
          docker rm -f "$NAME" >/dev/null 2>&1 || true
          NODE_ID_CLI="$NODE_MANUAL"
          run_container_instance "$NAME" "$LOGPATH"
          return
        fi
      fi
      err "Không lấy được node ID tự động."
      exit 1
    fi
  fi
}

# ===================== Run N containers (scale) =====================
run_containers() {
  local N="$1"
  [ "$N" -le 0 ] && N=1

  if [ "$N" -eq 1 ]; then
    run_container_instance "$(container_name_for_index 1 1)" "$(log_file_for_index 1 1)"
    return
  fi

  # Nếu chưa có node_id thì khởi chạy instance đầu tiên trước
  local HAVE_ID=0
  if [ -s "$NODE_ID_FILE" ] || [ -s "$CREDENTIALS_DIR/node_id" ] || [ -s "$CREDENTIALS_DIR/config.json" ] || [ -n "$NODE_ID_CLI" ]; then
    HAVE_ID=1
  fi
  if [ "$HAVE_ID" -eq 0 ]; then
    local name1; name1="$(container_name_for_index 1 "$N")"
    local log1;  log1="$(log_file_for_index 1 "$N")"
    inf "Chưa có node ID — khởi chạy $name1 trước để tạo ID..."
    run_container_instance "$name1" "$log1"
  fi

  local i
  for i in $(seq 1 "$N"); do
    local nm; nm="$(container_name_for_index "$i" "$N")"
    local lf; lf="$(log_file_for_index "$i" "$N")"
    run_container_instance "$nm" "$lf"
  done
}

# ===================== Watchdog: restart khi unhealthy/không chạy (đa container) =====================
watchdog() {
  ensure_pkgs
  ...
  for i in $(seq 1 "$N"); do
    ...
    fi
    {
      ...
      if [ "$status" = "running" ] && [ "$health" = "unhealthy" ]; then
        echo "restart"; docker restart "$NAME" >/dev/null 2>&1 || true; continue
      fi
      echo "(re)create $NAME"
      run_container_instance "$NAME" "$(log_file_for_index "$i" "$N")"
    } >> "$WATCHDOG_LOG" 2>&1
  done

  # Dọn container thừa > N
  local extras
  extras="$(docker ps -a --format '{{.Names}}' | grep -E "^${CONTAINER_NAME}-[0-9]+$" | sort || true)"
  if [ -n "$extras" ]; then
    local w; w="$(pad_width "$N")"
    local keep=""; local j
    for j in $(seq 1 "$N"); do
      [ -z "$keep" ] && keep="$(printf "%0${w}d" "$j")" || keep="$keep|$(printf "%0${w}d" "$j")"
    done
    while read -r nm; do
      echo "$nm" | grep -Eq "^${CONTAINER_NAME}-(${keep})$" && continue
      { echo "[$(date -Is)] remove extra $nm"; docker rm -f "$nm" >/dev/null 2>&1 || true; } >> "$WATCHDOG_LOG" 2>&1
    done <<< "$extras"
  fi
}

# ===================== Cập nhật theo tag mới (1h/lần, chỉ khi có) =====================
update_if_new() {
  ensure_pkgs
...
setup_cron() {
  ...
  # Lấy chuỗi tham số bền vững cho cron: ưu tiên node-id, rồi wallet
  ...
  local UP_EXPR="0 * * * *"; local UP_JOB="$UP_EXPR $UP_CMD"    # mỗi giờ
...
# ===================== Luồng chính =====================
# Yêu cầu tối thiểu: hoặc đã có NODE_ID (file/cli) hoặc có WALLET (file/cli)
has_any_input(){
...
case "$MODE" in
  watchdog)
    watchdog; exit 0 ;;
  update|smart-update)
    update_if_new; exit 0 ;;
  *)
    has_any_input || { err "$ERR_NO_INPUT"; exit 1; }
    latest_now="$(fetch_latest_tag)"
    if [ -z "$latest_now" ]; then warn "Không lấy được tag mới nhất, vẫn build theo thời điểm hiện tại."; latest_now="manual-$(date +%s)"; fi
