    ...
    case "${1:-}" in
        deploy)
            ...
            for ((i=1; i<=node_count; i++)); do
                if create_node "$i" && bind_node "titan-edge-$(printf "%02d" "$i")" "$hash_value"; then
                    ((success_count++))
                else
                    log_error "Failed to deploy node $i"
                fi
            done
            
            log_success "Successfully deployed $success_count/$node_count nodes"
            get_node_status
            ;;
