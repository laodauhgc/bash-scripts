    ...
    for project_id in $projects; do
        ...
        for zone in "${zones[@]}"; do
            ...
            gcloud compute instances create "$instance_name" \
                ...
                --metadata=startup-script="wget https://raw.githubusercontent.com/laodauhgc/bash-scripts/refs/heads/main/titan-network/gcp/install-agent.sh -4O install-agent.sh || curl https://raw.githubusercontent.com/laodauhgc/bash-scripts/refs/heads/main/titan-network/gcp/install-agent.sh -Lo install-agent.sh && bash install-agent.sh --key=$hash_value" \
