...
detect_os(){
  if [[ -f /etc/os-release ]]; then
    source /etc/os-release; OS="$ID"; OS_VERSION="$VERSION_ID"; OS_CODENAME="${VERSION_CODENAME:-}"
  elif [[ -f /etc/redhat-release ]]; then
    OS="centos"; OS_VERSION=$(grep -oE '[0-9]+\.[0-9]+' /etc/redhat-release | head -1)
  else
...
check_requirements(){
  ...
  local avail=$(df --output=avail -k / | tail -1)
  local need=$((DISK_MIN_GB*1024*1024))
  ((avail<need)) && fatal "Need $DISK_MIN_GB GB free"
  local mem=$(awk '/MemTotal/ {print $2}' /proc/meminfo)
  local needm=$((MEM_MIN_GB*1024*1024))
  ((mem<needm)) && log WARN "<${MEM_MIN_GB}GB RAM"
...
install_docker(){
  ...
  set_compose_cmd
  command -v systemctl &>/dev/null && systemctl enable --now docker &>/dev/null || service docker start &>/dev/null
...
install_compose(){
  ...
  if [[ -n "$COMPOSE_CMD" ]]; then
    local cur=$($COMPOSE_CMD version --short 2>/dev/null)
    if [[ -n "$cur" && $(version_ge "$cur" "$MIN_COMPOSE_VERSION" && echo yes) == yes ]]; then
      log INFO "Compose $cur OK"; return
    fi
  fi
  ...
  set_compose_cmd
  [[ -n "$COMPOSE_CMD" ]] || fatal "Compose not found"
...
restore_backup(){
  local f="$1"; [[ -f "$f" ]] || fatal "Backup $f missing"
...
#--------------------------------------
uninstall_blockcast(){
  set_compose_cmd; log INFO "Uninstalling"
...
main(){
  print_banner
  local action="install" dry=0 file=""
  while (( $# )); do
    case "$1" in
      -i|--install) action="install";;
      -u|--uninstall) action="uninstall";;
      -b|--backup) action="backup";;
      -r|--restore) action="restore"; file="$2"; shift;;
  ...
  case "$action" in
    install)
      ((dry)) && { log INFO "Dry run: install"; exit; }
      install_docker; install_compose; clone_repo; start_blockcast; generate_keys
      ;;
    uninstall)
      ((dry)) && { log INFO "Dry run: uninstall"; exit; }
      uninstall_blockcast
      ;;
    backup) create_backup;;
    restore) restore_backup "$file";;
    check) log INFO "System check complete";;
  esac
}
