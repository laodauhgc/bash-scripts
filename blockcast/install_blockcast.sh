...
check_root() {
  [[ $EUID -ne 0 ]] && fatal "Run as root (sudo $SCRIPT_NAME)"
}
...
check_requirements() {
  ...
  local need=$(( DISK_MIN_GB * 1024 * 1024 ))
  (( avail < need )) && fatal "Need at least ${DISK_MIN_GB}GB free on /"

  local mem
  mem=$(awk '/MemTotal/ {print $2}' /proc/meminfo)
  local needm=$(( MEM_MIN_GB * 1024 * 1024 ))
  (( mem < needm )) && log WARN "Detected < ${MEM_MIN_GB}GB RAM. Proceeding anyway."
...
install_docker() {
  if command -v docker &>/dev/null; then
    local cur
    cur=$(docker --version | grep -oE '[0-9]+\.[0-9]+\.[0-9]+' | head -1)
    if [[ -n "$cur" && $(version_ge "$cur" "$MIN_DOCKER_VERSION" && echo yes) == yes ]]; then
      log INFO "Docker $cur already meets requirement"
      return
    fi
    log WARN "Docker $cur outdated, installing via get.docker.com"
  fi

  log INFO "Installing Docker via get.docker.com"
  _fetch https://get.docker.com | sh || fatal "Failed to install Docker"
  ...
  [[ -n "${SUDO_USER:-}" ]] && usermod -aG docker "$SUDO_USER" && log INFO "Added $SUDO_USER to docker group" 
}
...
install_compose() {
  ...
  if [[ -n "$COMPOSE_CMD" ]]; then
    local cur
    cur=$($COMPOSE_CMD version --short 2>/dev/null)
    if [[ -n "$cur" && $(version_ge "$cur" "$MIN_COMPOSE_VERSION" && echo yes) == yes ]]; then
  ...
  case "$OS" in
    ubuntu|debian)
      apt-get update -qq && apt-get install -y docker-buildx-plugin docker-compose-plugin -qq
      ;;
    centos|rhel)
      yum install -y docker-buildx-plugin docker-compose-plugin -q
      ;;
    fedora)
      dnf install -y docker-buildx-plugin docker-compose-plugin -q
      ;;
    *) fatal "Compose plugin install unsupported on $OS" ;;  
  esac

  set_compose_cmd
  [[ -n "$COMPOSE_CMD" ]] || fatal "docker-compose command not found"
  log INFO "Docker Compose ready ($($COMPOSE_CMD version --short 2>/dev/null))"
}
...
create_backup() {
  ...
  if [[ -d "$HOME/.blockcast" ]]; then
    cp -a "$HOME/.blockcast" "$BACKUP_STAGING_DIR/"
  fi
  if [[ -d "$BEACON_DIR_NAME" ]]; then
    cp -a "$BEACON_DIR_NAME" "$BACKUP_STAGING_DIR/"
  fi
...
EOF
  ...
  local tarfile="$SCRIPT_DIR/${BACKUP_TAR_PREFIX}.tar.gz"
  tar -czf "$tarfile" -C "$(dirname "$BACKUP_STAGING_DIR")" "$(basename "$BACKUP_STAGING_DIR")"
  rm -rf "$BACKUP_STAGING_DIR"
  log INFO "Backup saved to $tarfile"
}

restore_backup() {
  local tarfile="$1"
  [[ -f "$tarfile" ]] || fatal "Backup file not found: $tarfile"
  log INFO "Restoring from $tarfile"
  local tmpdir="/tmp/restore_$$"
  mkdir -p "$tmpdir"
...
clone_repo() {
  ...
  if [[ -d "$BEACON_DIR_NAME" ]]; then
    mv "$BEACON_DIR_NAME" "${BEACON_DIR_NAME}.bak.$(date +%s)"
  fi
  git clone "$BEACON_REPO_URL" "$BEACON_DIR_NAME" || fatal "git clone failed"
}

start_blockcast() {
  log INFO "Starting Blockcast BEACON services..."
  pushd "$BEACON_DIR_NAME" &>/dev/null
  $COMPOSE_CMD pull || fatal "Failed to pull images"
  $COMPOSE_CMD up -d || fatal "Failed to start services"
  popd &>/dev/null
}
...
generate_keys() {
  ...
  pushd "$BEACON_DIR_NAME" &>/dev/null
  local out
  out=$($COMPOSE_CMD exec -T blockcastd blockcastd init 2>&1) || fatal "Init command failed"
  popd &>/dev/null
  ...
  [[ -n "$hwid" && -n "$challenge_key" ]] || fatal "Failed to parse init output"
...
uninstall_blockcast() {
  ...
  read -p "Remove Blockcast Docker images? [y/N]: " ans
