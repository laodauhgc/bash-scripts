...
# Version 2.0.14 (optimized package checking with dpkg -s for reliability)
# ==============================================================================
...
readonly SCRIPT_VERSION="2.0.14"
...
install_packages() {
    ...
    for package in "${CORE_PACKAGES[@]}"; do
        debug "Kiểm tra package: $package"
...
EOF
    ...
    [[ $CREATE_BACKUP -eq 1 ]] && info🔍 Chế độ dry-run được bật"
                shift
                ;;
            -s|--skip-nodejs)
                SKIP_NODEJS=1
...
post_installation_setup() {
    ...
    if systemctl list-unit-files --quiet | grep -q ssh; then
        systemctl enable ssh >/dev/null 2>&1 || true
        info "✅ SSH service enabled"
    fi
